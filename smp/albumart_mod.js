...
function _albumart (x, y, w, h) {
    this.paint = (gr) => {
        if (this.properties.cd.enabled) {
            ...
            if (this.img) {
                ...
                _drawImage(gr, this.img, nx, ny, nw, nh, this.properties.aspect.value, null, this.properties.alpha.value * 51);
            }
            ...
            _drawImage(gr, this.img, this.x, this.y, this.w, this.h, this.properties.aspect.value, null, this.properties.alpha.value * 51);
        }
    }
    ...
    this.rbtn_up = (x, y) => {
        ...
        panel.m.AppendMenuSeparator();
        _.forEach(this.alphas, (item, i) => {
            panel.m.AppendMenuItem(MF_STRING, i + 1020, item);
        });
        panel.m.CheckMenuRadioItem(1020, 1024, this.properties.alpha.value + 1019);
        panel.m.AppendMenuSeparator();
        _.forEach(this.aspects, (item, i) => {
            panel.m.AppendMenuItem(MF_STRING, i + 1030, item);
        });
        panel.m.CheckMenuRadioItem(1030, 1033, this.properties.aspect.value + 1030);
        panel.m.AppendMenuSeparator();
        panel.m.AppendMenuItem(_isFile(this.path) ? MF_STRING : MF_GRAYED, 1040, 'Open containing folder');
        panel.m.AppendMenuSeparator();
        panel.m.AppendMenuItem(panel.metadb ? MF_STRING : MF_GRAYED, 1050, 'Google image search');
        panel.m.AppendMenuSeparator();
    }
    ...
    this.rbtn_up_done = (idx) => {
        ...
        case 1023:
        case 1024:
            this.properties.alpha.value = (idx - 1019);
            window.RepaintRect(this.x, this.y, this.w, this.h);
            break;
        case 1030:
        case 1031:
        case 1032:
        case 1033:
            this.properties.aspect.value = idx - 1030;
            window.RepaintRect(this.x, this.y, this.w, this.h);
            break;
        case 1040:
            _explorer(this.path);
    ...
    this.alphas = ['Opacity 20%', 'Opacity 40%', 'Opacity 60%', 'Opacity 80%', 'Opacity 100%']
    this.images = {
    ...
    this.properties = {
        alpha : new _p('2K3.ARTREADER.ALPHA', 5),
